...
// ============================================================
// RILEVAMENTO DATA SCADENZA TRAMITE OCR (Tesseract.js)
// ============================================================
var expiryOCRWorker = null;
var detectedExpiryDate = null;
var detectedExpiryConfidence = 0;
var expiryScanFrameInterval = null;
var isExpiryScanning = false;

/**
 * Inizializza il worker Tesseract.js per OCR data
 */
function initExpiryOCR() {
  ...
  return new Promise(function(resolve, reject) {
    if (typeof Tesseract === 'undefined') {
      console.warn('Tesseract.js non disponibile');
...
/**
 * Avvia la scansione continua della data di scadenza
 * La camera resta attiva, cattura frame periodicamente
 */
function scanExpiryDate() {
  if (!isScanning) {
    showToast('&#9888;&#65039; Avvia prima lo scanner');
    return;
  }
  ...
  btn.innerHTML = '&#9209; Ferma scansione';
  isExpiryScanning = true;

  showToast('&#128247; Inquadra la data di scadenza...');

  initExpiryOCR()
    .then(function() {
      // Avvia cattura frame ogni 1.5 secondi
      expiryScanFrameInterval = setInterval(captureAndAnalyzeExpiry, 1500);
      // Prima cattura immediata
      captureAndAnalyzeExpiry();
    })
    .catch(function(err) {
      statusDiv.style.display = 'none';
      btn.innerHTML = '&#128247; Scansiona data scadenza';
      isExpiryScanning = false;
      showToast('&#10060; OCR non disponibile: ' + err.message);
    });
}

/**
 * Ferma la scansione data scadenza
 */
function stopExpiryScan() {
  isExpiryScanning = false;
  if (expiryScanFrameInterval) {
    clearInterval(expiryScanFrameInterval);
    expiryScanFrameInterval = null;
  }
  var statusDiv = document.getElementById('expiry-scan-status');
  var btn = document.getElementById('btn-scan-expiry');
  if (statusDiv) statusDiv.style.display = 'none';
  if (btn) btn.innerHTML = '&#128247; Scansiona data scadenza';
}

/**
 * Cattura un frame dalla camera e analizza con OCR
 */
function captureAndAnalyzeExpiry() {
  ...
  // Cattura frame a risoluzione ridotta per velocita
  var scale = 0.5;
  canvas.width = video.videoWidth * scale;
  canvas.height = video.videoHeight * scale;
  ctx.drawImage(video, 0, 0, canvas.width, canvas.height);

  // Preprocessing: grayscale + contrasto per migliorare OCR
  var imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
  var data = imageData.data;
  for (var i = 0; i < data.length; i += 4) {
    var gray = 0.299 * data[i] + 0.587 * data[i+1] + 0.114 * data[i+2];
    // Aumenta contrasto
    gray = gray < 128 ? gray * 0.7 : Math.min(255, gray * 1.3);
  ...
  // Analizza con Tesseract - whitelist solo cifre, slash, punti, trattini
  expiryOCRWorker.recognize(dataUrl, {}, { 
    tessedit_char_whitelist: '0123456789/.-',
    psm: 6  // Assume singolo blocco di testo uniforme
  })
    .then(function(result) {
      ...
      if (date && confidence > 30) {
        ...
        // Ferma scansione e mostra conferma
        stopExpiryScan();
        showExpiryConfirmModal(dataUrl, date, confidence);
      }
    })
    .catch(function(err) {
 ...
 * Supporta formati: DD/MM/YYYY, DD-MM-YYYY, YYYY/MM/DD, YYYY-MM-DD, 
 *                  DD.MM.YYYY, MM/DD/YYYY, ecc.
 */
function extractDateFromText(text) {
  if (!text) return null;

  // Pulisci il testo: rimuovi spazi extra
  var clean = text.replace(/\s+/g, '');

  // Pattern per date comuni su imballaggi
  var patterns = [
    // DD/MM/YYYY o DD-MM-YYYY o DD.MM.YYYY
    { regex: /(\d{1,2})[\/.\-](\d{1,2})[\/.\-](\d{4})/, format: 'DMY' },
    // YYYY/MM/DD o YYYY-MM-DD
    { regex: /(\d{4})[\/.\-](\d{1,2})[\/.\-](\d{1,2})/, format: 'YMD' },
    // DD/MM/YY o DD-MM-YY (anno a 2 cifre)
    { regex: /(\d{1,2})[\/.\-](\d{1,2})[\/.\-](\d{2})/, format: 'DMY_SHORT' },
    // MM/DD/YY
    { regex: /(\d{1,2})[\/.\-](\d{1,2})[\/.\-](\d{2})/, format: 'MDY_SHORT' }
  ];
  ...
  for (var i = 0; i < patterns.length; i++) {
    ...
    if (match) {
      ...
      } else if (patterns[i].format === 'MDY_SHORT') {
        m = parseInt(match[1], 10);
        d = parseInt(match[2], 10);
        y = parseInt(match[3], 10) + 2000;
      }

      // Validazione base
      if (d >= 1 && d <= 31 && m >= 1 && m <= 12 && y >= 2020 && y <= 2040) {
        // Formatta come YYYY-MM-DD per input type="date"
        var yy = String(y);
...
function showExpiryConfirmModal(previewUrl, dateStr, confidence) {
  ...
  previewDiv.innerHTML = '<img src="' + previewUrl + '" style="width:100%;height:100%;object-fit:cover;" alt="Frame scansionato">';
...
/**
 * Accetta la data rilevata e la inserisce nel form
 */
...
/**
 * Modifica manualmente la data rilevata
 */
...
/**
 * Termina il worker Tesseract quando non serve piu
 */
...
// ============================================================
// RILEVAMENTO DATA SCADENZA - FOTO SINGOLA CON TESSERACT.js
// ============================================================
var expiryOCRWorker = null;
var detectedExpiryDate = null;
var detectedExpiryConfidence = 0;
...
/**
 * Avvia la fotocamera per scattare foto alla data di scadenza
 */
function scanExpiryDate() {
  var btn = document.getElementById('btn-scan-expiry');
  var statusDiv = document.getElementById('expiry-scan-status');
  ...
  showToast('&#128247; Avvio fotocamera per data scadenza...');
...
/**
 * Cattura la foto alla data di scadenza
 */
function captureExpiryPhoto() {
  ...
  // Preprocessing per OCR: ritaglia area centrale (dove c'è la data)
  // e applica grayscale + contrasto
  var cropCanvas = document.createElement('canvas');
  var cropCtx = cropCanvas.getContext('2d');
  
  // Ritaglia zona centrale (dove l'utente ha inquadrato la data)
  ...
  initExpiryOCR()
    .then(function() {
      return expiryOCRWorker.recognize(ocrImageData, {}, { 
        tessedit_char_whitelist: '0123456789/.-',
        psm: 6
      });
    })
    .then(function(result) {
      ...
      if (date && confidence > 25) {
        ...
        // Nessuna data trovata, mostra comunque preview per riprovare
        showToast('&#128533; Data non rilevata, riprova o inserisci manualmente');
        detectedExpiryDate = null;
        detectedExpiryConfidence = 0;
        // Mostra modal con possibilità di riprovare
    ...
    .catch(function(err) {
      document.getElementById('expiry-ocr-loading').classList.remove('active');
      console.error('Errore OCR:', err);
      showToast('&#10060; Errore analisi: ' + err.message);
      closeExpiryCamera();
    });
}
...
function showExpiryConfirmModal(previewUrl, dateStr, confidence) {
  ...
  if (dateStr) {
    ...
    confDiv.textContent = 'Prova a scattare di nuovo o inserisci manualmente';
    confDiv.style.display = 'block';
  }
