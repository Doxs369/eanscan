...
var selectedCategory = 'pantry'; // Categoria selezionata per il prodotto in inserimento

var categoryEmojis = {
  dairy: '&#129371;', meat: '&#129385;', produce: '&#129388;',
  pantry: '&#129387;', beverages: '&#129380;', frozen: '&#129482;',
  bakery: '&#127838;', sweets: '&#127852;', fridge: '&#129379;',
  condiments: '&#129474;'
};
...
var categoryNames = {
  ...
  bakery: 'Panetteria', sweets: 'Dolci', fridge: 'Frigorifero',
  condiments: 'Condimenti'
};

/**
 * Ottieni emoji per categoria (inclusi quelli personalizzati)
 */
function getCategoryEmojiDynamic(category) {
  if (categoryEmojis[category]) return categoryEmojis[category];
  var all = OpenFoodFacts.getAllCategories();
  if (all[category]) return all[category].emoji;
  return '&#128230;';
}

/**
 * Ottieni nome per categoria (inclusi quelli personalizzati)
 */
function getCategoryNameDynamic(category) {
...
function handleManualProductPhoto(event) {
  ...
  var processPhoto = function(dataUrl) {
    ...
    // Aggiorna selettore categoria
    updateCategorySelector('pantry', '&#129387;');
...
function showProductFound(product) {
  ...
  // Aggiorna selettore categoria con quella rilevata
  updateCategorySelector(category, emoji);
...
function showProductNotFound(barcode) {
  ...
  // Aggiorna selettore categoria con default pantry
  updateCategorySelector('pantry', '&#129387;');
...
function addProduct() {
  ...
  var finalCategory = catInput ? catInput.value : selectedCategory;
  ...
  var catEmoji = OpenFoodFacts.getCategoryEmoji(finalCategory);

  var newProduct = {
    id: nextId++,
    name: name,
    emoji: catEmoji || scannedProductData.emoji || '&#128230;',
    category: finalCategory || 'pantry',
    expiryDate: expiry,
    qty: qty,
...
function sortProducts(list) {
  ...
  sorted.sort(function(a, b) {
    ...
    if (currentSort === 'category') {
      var catA = getCategoryNameDynamic(a.category);
      var catB = getCategoryNameDynamic(b.category);
...
function openProductModal(id) {
  ...
  var catEmoji = getCategoryEmojiDynamic(product.category);
  var catName = getCategoryNameDynamic(product.category);
...
/**
 * Scatta la foto e analizza la data con OCR
 */
...
/**
 * Analizza l'immagine con Tesseract.js v5 (una sola volta)
 */
...
/**
 * Aggiorna il selettore categoria nell'UI di inserimento
 */
function updateCategorySelector(category, emoji) {
  selectedCategory = category;
  var catName = OpenFoodFacts.getCategoryName(category);
  var catEmoji = emoji || OpenFoodFacts.getCategoryEmoji(category);

  var emojiEl = document.getElementById('cat-emoji');
  var nameEl = document.getElementById('cat-name');
  var inputEl = document.getElementById('category-input');

  if (emojiEl) emojiEl.innerHTML = catEmoji;
  if (nameEl) nameEl.textContent = catName;
  if (inputEl) inputEl.value = category;
}

/**
 * Apre il modal per la selezione categoria
 */
...
function closeCategoryModalDirect() {
  document.getElementById('category-modal').classList.remove('show');
}

/**
 * Renderizza la lista delle categorie nel modal
 */
function renderCategoryList() {
  var list = document.getElementById('category-list');
  if (!list) return;

  var allCats = OpenFoodFacts.getAllCategories();
  var html = '';

  for (var key in allCats) {
    var cat = allCats[key];
    var isActive = key === selectedCategory;
    html +=
      '<div class="category-option ' + (isActive ? 'active' : '') + '" onclick="selectCategory(&#39;' + key + '&#39;)">' +
        '<div class="category-option-emoji">' + cat.emoji + '</div>' +
        '<div class="category-option-text">' + cat.name + '</div>' +
        '<div class="category-option-check ' + (isActive ? '' : 'hidden') + '">&#10003;</div>' +
      '</div>';
  }
...
/**
 * Seleziona una categoria esistente
 */
function selectCategory(key) {
  selectedCategory = key;
  var cat = OpenFoodFacts.getAllCategories()[key];
  if (cat) {
    updateCategorySelector(key, cat.emoji);
  }
  closeCategoryModalDirect();
  showToast('&#128451; Categoria: ' + (cat ? cat.name : key));
}
...
function selectEmoji(el) {
  var options = document.querySelectorAll('.emoji-option');
  for (var i = 0; i < options.length; i++) {
    options[i].classList.remove('active');
  }
  el.classList.add('active');
  document.getElementById('new-cat-emoji').value = el.getAttribute('data-emoji');
}
...
function createNewCategory() {
  ...
  var emojiInput = document.getElementById('new-cat-emoji');
  ...
  var key = 'custom_' + name.toLowerCase().replace(/[^a-z0-9]/g, '_') + '_' + Date.now();

  OpenFoodFacts.addCustomCategory(key, name, emoji);

  // Seleziona subito la nuova categoria
  selectedCategory = key;
  updateCategorySelector(key, emoji);
  ...
  // Aggiorna anche le chip filtro nella dispensa
  updateFilterChips();
}

/**
 * Aggiorna le chip filtro nella schermata dispensa
 */
function updateFilterChips() {
  ...
  // Mantieni le chip base + expiring
  var baseCats = ['all', 'dairy', 'meat', 'produce', 'pantry', 'expiring'];
  var customCats = [];

  for (var key in allCats) {
    if (baseCats.indexOf(key) === -1 && key !== 'expiring') {
      customCats.push(key);
    }
  }

  var html =
    '<div class="chip ' + (currentFilter === 'all' ? 'active' : '') + '" data-cat="all" onclick="filterProducts(&#39;all&#39;, this)">Tutti</div>' +
    '<div class="chip ' + (currentFilter === 'dairy' ? 'active' : '') + '" data-cat="dairy" onclick="filterProducts(&#39;dairy&#39;, this)">&#129371; Latticini</div>' +
    '<div class="chip ' + (currentFilter === 'meat' ? 'active' : '') + '" data-cat="meat" onclick="filterProducts(&#39;meat&#39;, this)">&#129385; Carne</div>' +
    '<div class="chip ' + (currentFilter === 'produce' ? 'active' : '') + '" data-cat="produce" onclick="filterProducts(&#39;produce&#39;, this)">&#129388; Verdura</div>' +
    '<div class="chip ' + (currentFilter === 'pantry' ? 'active' : '') + '" data-cat="pantry" onclick="filterProducts(&#39;pantry&#39;, this)">&#129387; Dispensa</div>';

  // Aggiungi categorie personalizzate
  for (var i = 0; i < customCats.length; i++) {
    var k = customCats[i];
    var c = allCats[k];
    html += '<div class="chip ' + (currentFilter === k ? 'active' : '') + '" data-cat="' + k + '" onclick="filterProducts(&#39;' + k + '&#39;, this)">' + c.emoji + ' ' + c.name + '</div>';
  }

  html += '<div class="chip ' + (currentFilter === 'expiring' ? 'active' : '') + '" data-cat="expiring" onclick="filterProducts(&#39;expiring&#39;, this)">&#9200; In scadenza</div>';
