...
/**
 * Avvia la cattura automatica di N frame per trovare la data
 */
...
function startAutoCapture(maxAttempts) {
  autoCaptureMax = maxAttempts;
  autoCaptureCount = 0;
  autoCaptureFound = false;

  updateExpiryCameraStatus('Analisi automatica in corso... (0/' + autoCaptureMax + ')');

  loadTesseractJS().then(function() {
    return initExpiryOCR();
  }).then(function() {
    autoCaptureInterval = setInterval(function() {
      if (autoCaptureFound || autoCaptureCount >= autoCaptureMax) {
        clearInterval(autoCaptureInterval);
        autoCaptureInterval = null;

        if (!autoCaptureFound) {
          showManualCaptureOption();
        }
        return;
      }

      autoCaptureCount++;
      updateExpiryCameraStatus('Analisi automatica in corso... (' + autoCaptureCount + '/' + autoCaptureMax + ')');
...
/**
 * Cattura un singolo frame dalla camera come DataURL
 */
...
/**
 * Prova l'OCR su un singolo frame
 */
...
/**
 * Aggiorna il testo di stato nell'overlay camera
 */
...
/**
 * Mostra l'opzione di cattura manuale quando l'auto-scan fallisce
 */
function showManualCaptureOption() {
  ...
  // Mostra hint per scatto manuale
  var hintEl = document.querySelector('.expiry-camera-hint');
  if (hintEl) {
    hintEl.innerHTML = 'Nessuna data rilevata automaticamente<br><strong>Scatta tu la foto</strong>';
  }
}

/**
 * Cattura manuale quando l'auto-scan fallisce
 */
function manualExpiryCapture() {
  ...
  updateExpiryCameraStatus('Analizzo la foto...');
...
/**
 * Sistema di scansione data scadenza con 10 tentativi automatici + fallback manuale
 */
var expiryAutoAttempts = 0;
var expiryMaxAttempts = 10;
var expiryAutoInterval = null;
var expiryIsAutoScanning = false;

/**
 * Avvia la scansione automatica della data (10 tentativi)
 */
function scanExpiryDate() {
  ...
  showToast('&#128247; Avvio scansione automatica data...');
  ...
  .then(function(stream) {
    ...
    // Reset stato
    expiryAutoAttempts = 0;
    expiryIsAutoScanning = true;

    // Aggiorna UI
    updateExpiryCameraStatus('Scansione automatica in corso...');
    document.getElementById('expiry-camera-hint').innerHTML = 'Tieni ferma la data nell\'area tratteggiata<br>Sto cercando automaticamente...';

    // Avvia i tentativi automatici
    startExpiryAutoScan();
...
/**
 * Avvia il loop di tentativi automatici
 */
function startExpiryAutoScan() {
  if (!expiryIsAutoScanning) return;

  // Primo tentativo immediato
  attemptExpiryAutoCapture();

  // Poi ogni 800ms
  expiryAutoInterval = setInterval(function() {
    if (!expiryIsAutoScanning) {
      clearInterval(expiryAutoInterval);
      expiryAutoInterval = null;
      return;
    }
    attemptExpiryAutoCapture();
  }, 800);
...
/**
 * Esegue un singolo tentativo automatico di cattura e OCR
 */
function attemptExpiryAutoCapture() {
  if (!expiryIsAutoScanning) return;
  ...
  updateExpiryCameraStatus('Tentativo ' + expiryAutoAttempts + '/' + expiryMaxAttempts);
  ...
  // Salva ultima foto per fallback manuale
  expiryPhotoData = canvas.toDataURL('image/jpeg', 0.9);

  // Analizza silenziosamente (senza mostrare loading)
  analyzeExpiryImageSilent(ocrImageData, function(date, confidence) {
    if (!expiryIsAutoScanning) return;

    if (date) {
      // Data trovata! Ferma tutto e mostra conferma
      stopExpiryAutoScan();
      closeExpiryCamera();
      detectedExpiryDate = date;
      detectedExpiryConfidence = confidence || 70;
      showExpiryConfirmModal(expiryPhotoData, date, detectedExpiryConfidence);
      setExpiryButtonLoading(false);
      showToast('&#9989; Data rilevata al tentativo ' + expiryAutoAttempts + '!');
      return;
    }

    // Data non trovata, controlla se abbiamo esaurito i tentativi
    if (expiryAutoAttempts >= expiryMaxAttempts) {
      stopExpiryAutoScan();
      // Passa alla modalità manuale
      switchToManualExpiryMode();
    }
  });
}

/**
 * Analizza immagine con OCR in modalità silenziosa (no UI loading)
 */
function analyzeExpiryImageSilent(ocrImageData, callback) {
  initExpiryOCR()
    .then(function() {
    ...
    .then(function(result) {
      ...
      var confidence = (result.data && result.data.confidence) ? result.data.confidence : 0;
      var date = extractDateFromText(text);
      callback(date, confidence);
    })
    .catch(function(err) {
      console.error('Errore OCR silenzioso:', err);
      callback(null, 0);
    });
}

/**
 * Ferma il loop di scansione automatica
 */
function stopExpiryAutoScan() {
  expiryIsAutoScanning = false;
  if (expiryAutoInterval) {
    clearInterval(expiryAutoInterval);
    expiryAutoInterval = null;
  }
}
...
/**
 * Passa alla modalità manuale dopo i tentativi automatici falliti
 */
function switchToManualExpiryMode() {
  updateExpiryCameraStatus('Inquadra la data e scatta la foto');
  document.getElementById('expiry-camera-hint').innerHTML = 'Non ho trovato la data automaticamente<br>Inquadra la data e scatta la foto manualmente';

  // Mostra il pulsante di scatto manuale (se nascosto)
  var actionsDiv = document.querySelector('.expiry-camera-actions');
  if (actionsDiv) {
...
/**
 * Aggiorna il testo di stato nella camera overlay
 */
function updateExpiryCameraStatus(text) {
  var statusEl = document.getElementById('expiry-camera-status');
  if (statusEl) {
...
/**
 * Scatta la foto manualmente (fallback dopo tentativi automatici)
 */
function captureExpiryPhoto() {
  // Se stiamo ancora facendo tentativi automatici, fermali
  if (expiryIsAutoScanning) {
    stopExpiryAutoScan();
  }
...
/**
 * Analizza l'immagine con Tesseract.js v5 (una sola volta, manuale)
 */
