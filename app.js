...
// ============================================================
// RILEVAMENTO DATA SCADENZA - SISTEMA IBRIDO
 ...
 * Fase 1: scansione automatica | Fase 2: foto singola
 */
...
function startAutoExpiryScan() {
  ...
  initExpiryOCR()
    ...
    .catch(function(err) {
      ...
      // Se OCR non disponibile, vai diretto a foto singola
      showToast('&#128247; OCR non pronto, passo a foto manuale');
...
function analyzeExpiryFrameAuto() {
  ...
  expiryOCRWorker.recognize(ocrImageData, {}, { text: true })
    .then(function(result) {
      ...
      // Se max tentativi raggiunti, passa a foto singola
    ...
    .catch(function(err) {
      ...
      // Se errore e abbiamo fatto abbastanza tentativi, passa a manuale
      if (ocrAttemptCount >= maxAutoAttempts) {
        stopExpiryScan();
        switchToManualCapture();
      }
 ...
 * L'utente scatta la foto quando vuole
 */
function switchToManualCapture() {
  ...
  // La fotocamera resta aperta, l'utente clicca il tasto di scatto
}
