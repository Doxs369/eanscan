...
var OpenFoodFacts = (function() {
  ...
  /**
   * Recupera TUTTI i dettagli di un prodotto per barcode
   * Usato dal tasto Info nel modal prodotto
   */
  function getFullDetails(barcode) {
    return new Promise(function(resolve, reject) {
      var url = API_BASE + encodeURIComponent(barcode) + '.json';
      ...
      fetch(url)
        ...
        .then(function(data) {
          if (data.status === 1 && data.product) {
            ...
            if (p.nutriments) {
              nutrientsServing = {
                energyKcal: n['energy-kcal_serving'] || '',
                fat: n['fat_serving'] || '',
                saturatedFat: n['saturated-fat_serving'] || '',
                carbohydrates: n['carbohydrates_serving'] || '',
                sugars: n['sugars_serving'] || '',
                proteins: n['proteins_serving'] || '',
                salt: n['salt_serving'] || ''
              };
            }

            // Nutri-Score dettagliato
            var nutriScoreData = p.nutriscore || {};
            var nutriScoreGrade = p.nutriscore_grade || '';
            var nutriScoreScore = p.nutriscore_score !== undefined ? p.nutriscore_score : '';

            // Nova Group
            var novaGroup = p.nova_group || '';

            // Eco-Score
            var ecoScore = p.ecoscore_grade || '';

            // Livelli nutrizionali (traffic lights)
            var nutrientLevels = p.nutrient_levels || {};

            // Allergeni
            var allergens = [];
            if (p.allergens_tags) {
              allergens = p.allergens_tags.map(function(a) {
                return a.replace('en:', '').replace('it:', '');
              });
            }
            ...
            var traces = [];
            if (p.traces_tags) {
              traces = p.traces_tags.map(function(t) {
                return t.replace('en:', '').replace('it:', '');
              });
            }
            ...
            var additives = [];
            if (p.additives_tags) {
              additives = p.additives_tags.map(function(a) {
                return a.replace('en:', '').replace('it:', '');
              });
            }
            ...
            var labels = [];
            if (p.labels_tags) {
              labels = p.labels_tags.map(function(l) {
                return l.replace('en:', '').replace('it:', '');
              });
            }
            ...
            var details = {
              name: p.product_name || p.product_name_it || p.product_name_en || 'Prodotto sconosciuto',
              brand: p.brands || '',
              quantity: p.quantity || '',
              barcode: barcode,
              ...
              origins: p.origins || p.origins_it || '',
            ...
            resolve({ found: true, details: details });
          } else {
            resolve({ found: false, error: 'Prodotto non trovato' });
          }
        })
        .catch(function(error) {
          console.error('Errore dettagli OFF:', error);
          resolve({ found: false, error: error.message || 'Errore di rete' });
        });
    });
  }

  /**
   * Ottieni colore per livello nutrizionale
   */
  ...
  /**
   * Ottieni nome italiano per livello
   */
  ...
  /**
   * Ottieni colore Nutri-Score
   */
  function getNutriScoreColor(grade) {
    var colors = { a: '#008b4c', b: '#82c357', c: '#fecb02', d: '#ef8200', e: '#e63e0f' };
    return colors[grade] || '#888';
  }

  /**
   * Ottieni colore Nova Group
   */
  function getNovaColor(group) {
    var colors = { 1: '#00aa00', 2: '#ffcc00', 3: '#ff6600', 4: '#ff0000' };
